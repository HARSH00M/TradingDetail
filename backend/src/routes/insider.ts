...
const companywise = (datefrom: string, dateto: string, fromamount: string, toamount: string) => {
    return sql`
      ...
      WHERE 
          ...
          ${
            fromamount && toamount 
            ? sql`AND ts.total_value_of_security_acquired_disposed BETWEEN ${fromamount}::numeric AND ${toamount}::numeric`
            : sql``
          }
      ORDER BY 
...
router.post('/companywise', async (req: Request, res) => {
    ...
    try {
        console.log(fromdate, todate)
        const data = await companywise(fromdate, todate, fromamount, toamount);
        res.json({
